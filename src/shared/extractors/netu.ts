...
export class NetuE extends VideoExtractor {
  ...
  override extract = async (): Promise<ISource> => {
    ...
    const image_res: PlayerImage | PlayerImageFail = await request(hostname + "/player/get_player_image.php", "POST",
        {
    ...
    const playButtonCoords: number[] = await request("https://mochi_back.nixuge.me/epicImageFinder", "POST",
      {"Content-Type": "application/json"},
      JSON.stringify({"b64image": image_res.image})
    ).then(req => req.json());
    ...
    const finalResp: DataResponse = await request(hostname + "/player/get_md5.php", "POST",
      headers,
      JSON.stringify(body)
    ).then(resp => resp.json());
