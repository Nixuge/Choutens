...
import { DiscoverData, DiscoverListing, DiscoverListings, DiscoverTypes } from "../../types";
...
export class HomeScraper {
    ...
    async scrape(): Promise<DiscoverData> {
      const html = await request(`${this.baseName}/home`, "GET").then(resp => resp.body);
      
      this.$ = cheerio.load(html);

      return {listings: this.scrapeAll()};
    }
    ...
    private scrapeHotest(): DiscoverListings {
        ...
        const hotestItems: DiscoverListing[] = swiper.map((_i, anime) => {
            const animeRef = $(anime)
            const title = animeRef.find("h2.title.d-title").text();
            let image: string | null | undefined = animeRef.find("div.image > div").attr("style")?.replace("background-image: url('", "").replace("');", "");
            if (image == undefined) { // Dirty fix to make TS happy for chouten (not sure if it rly affects it as cant test rn)
              image = null;
            }
    ...
    private scrapeRecentlyUpdated(): DiscoverListings {
        ...
        const recentlyUpdatedItems: DiscoverListing[] = recentUpdatesSec.map((_i, anime) => {
          const animeRef = $(anime);
          const titleElem = animeRef.find("a.name.d-title");
          const title = titleElem.text();
          const url = titleElem.attr("href")!;
          let image: string | null | undefined = animeRef.find("div.ani.poster > a > img").attr("src");     
    ...
    private scrapeBottomThreeColumns(id: string): DiscoverListings {
      const table = this.$("div.top-tables.mb-3 .body .top-table").filter((_i, _table) => {
      ...
      const items: DiscoverListing[] = $("div.body div.scaff.items a.item").map((_i, item) => {
        const itemRef = $(item);
        
        const url = itemRef.attr("href")!;
        let image: string | null | undefined = itemRef.find("div.poster span img").attr("src")?.replace("-w100", "-w9999"); // higher quality
        if (image == undefined) { image = null; } 
